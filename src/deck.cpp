    ...
    )
{
    ...
    /* Relative ratios of stat points available to creatures in each tier */
    static constexpr const std::array<std::pair<float, float>, Creature::numTiers> tierQualityThresholds = {{
            {0.50f, 0.56f},
            {0.56f, 0.63f},
            {0.63f, 0.70f},
            {0.70f, 0.80f},
    ...
    std::uniform_real_distribution<float> dist(
            tierQualityThresholds.at(tierNum).first,
            tierQualityThresholds.at(tierNum).second
        );
...
std::vector<int> DeckBuilder::_num_cards_per_tier(int totalMaxNumCards)
{
    ...
    for (const float& ratio: tierVolumeRatios)
    {
